...
class OCVThread2( Threaded ):
	...
	def update_frame( self, shapes ):
		...
		lock.acquire()
		gui.ShowImage( 'contours', self.contours_image() )
		gui.WaitKey(1)	# waitkey is called in other ocv thread
		lock.release()
...
class OCVThread1( Threaded ):
	...
	def __init__(self):
		...
		self.sweep_thresh = [ cv.CreateImage((640,480), 8, 1) for i in range(SWEEPS) ]
	...
	def loop(self):
		...
		while self.active:
			...
			gui.ShowImage( 'depth', self.depth8 )
			gui.WaitKey(1)
			lock.release()
...
#cv.ClearMemStorage( mem )	# segfaults
print( 'testing releasing mem' )
cv.ReleaseMemStorage( ctypes.pointer(mem.POINTER) )
print( 'mem released ok.' )
